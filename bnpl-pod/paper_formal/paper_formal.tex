...
It Through Structured-Credit Derivatives\\[0.3em]
\normalsize\mdseries\itshape Formal Academic Draft}

\author{
Siddharth~Verma \\[0.3em]
\small University of Illinois Urbana-Champaign \\
\small FIN/CS 580 --- Spring 2026}
\date{April 2026 \\ \small\textit{Formal draft --- prepared for AI-mediated academic review}}
...
median on the single day $\bsi$ registers a $+27.4\sigma$ spike in
...
Viewed at aggregate, this constitutes a \emph{Micro-Leverage Epoch}: thousands of
...
The thesis of this paper --- that BNPL credit exhibits the
structural features of a Subprime-2.0 opacity regime --- is the
...
anticipating the Reg~Z event one day early via a pre-deadline
complaint surge) and $+7.93$ on 25~February~2021 (MOVE spike during
the retail-flow-driven short-squeeze episode). The Reg~Z $+27.4\sigma$ reading is an
...
--- and neither the bureau, nor the public-ABS auto securitisation
complex, nor the broad HY-credit market re-prices on that signal at a 4--8
...
to $+27.4\sigma$ (driven by 12{,}838 BNPL complaints in a single day
...
(17~January~2025, $+27.4\sigma$), implying a false-positive budget of
...
the entire 2019--2026 BSI history, with BSI $z = +27.4\sigma$ driven by
...
fires on 4 days with maximum $z = +27.4\sigma$, Gate~3 (catalyst) fires
...
\item \textsc{Reg~Z}: Gate~1 fires at extreme magnitude --- BSI $z$ reaches
$+27.4\sigma$ on 17~January~2025, driven by 12{,}838 BNPL complaints
...
$\bsi_z$ peaked at $+27.4\sigma$. No other day in the sample
...
retail-flow-driven MOVE spike, one standard-deviation short of the bypass).
...
17~January~2025) triggers the BSI $+27.4\sigma$ spike, gates fire, and
...
This counterfactual constitutes the paper's primary empirical
result. It serves two purposes simultaneously:
\begin{itemize}[leftmargin=*,itemsep=2pt]
\item \textbf{Validates the BSI}. The composite fires
the single largest event-window signal in the 2022--2025 sample
($+27.4\sigma$ on 17~January~2025) on a date that is independently
verifiable as the peak BNPL stress event (12{,}838 CFPB complaints
in a single day). While the warehouse also records a broader
all-history peak of +44.03$\sigma$ on a distinct out-of-sample
date, the event-study economics reported in this paper are
identified on the in-window Reg~Z peak. The signal is
...
points over 40 trading days. The equity expression is subject to
reversal driven by retail-flow dynamics unrelated to the BNPL
fundamentals signal; the TRS expression tracks the spread-widening
and roll-rate channel that the BSI informs.
\end{itemize}
...
largest signal in the warehouse ($z = +27.4\sigma$). Distribution
...
broader macroeconomic environment that did not corroborate until
months later.
...
proxy) is, considered in isolation, not a powered test against
the strict alternative of non-zero predictive content. The null
result is framed here as corroborative of the Subprime-2.0
thesis --- that BNPL-specific consumer stress is orthogonal to
broad high-yield credit stress at weekly horizons --- rather
than as a primary empirical claim. The paper's primary empirical
claims --- the $+5.33$ percentage-point cumulative spread between
the TRS-on-junior-ABS expression and the naive-equity short
reported in Table~\ref{tab:blindspot}, and the single
$+27.4\sigma$ super-threshold bypass event of 17~January~2025 ---
rest on event-window economics that are identified on a known
catalyst date, not on rejection of any aggregate $p$-value. The
multiple-comparisons exposure from testing five lags against two
target series is acknowledged; a Bonferroni-adjusted rejection
...
interpretation of the $+27.4\sigma$ Regulation~Z spike: the spike
documents an escalation in dispute-channel activity among
agency-retaining consumers, not a proportional escalation in
realised default rates across the full distressed population.
